...
class Lessons(object):
    ...
    def __init__(self, file_name, duration = 365):
        ...
        lesson_lines = [x.strip() for x in lesson_lines if x.strip()
        ...
        self.lesson_lines = [x for x in lesson_lines
                            if not x.lower().startswith('lessonrate') and
                            not x.lower().startswith('setday') and
                            not x.lower().startswith('allowableintrodays')]
